...
import { searchDocuments, getStats, getSearchHistory } from './lib/documents'
...
function getRelVar(score: number): string {
  if (score >= 80) return 'var(--rel-90)'
  if (score >= 60) return 'var(--rel-70)'
  return 'var(--rel-50)'
}

function highlightExcerpt(text: string, query: string): ExcerptPart[] {
...
export default function App() {
  ...
  useEffect(() => {
    getStats().then(s => setDocCount(s.total_documents || 0)).catch(() => {})
    getSearchHistory(5).then(h => setRecentQueries(h.map((r: any) => r.query))).catch(() => {})
  }, [])
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (apiResults.length === 0) {
        ...
        const mapped: Result[] = apiResults.map(r => {
          const rel = Math.round(r.maxSimilarity * 100)
          const bestChunk = r.chunks[0]
          const excerptText = bestChunk?.content?.slice(0, 300) || ''
          return {
            relevance: rel,
            relVar: getRelVar(rel),
            docType: (r.doc_type || 'HUJJAT').toUpperCase(),
            docNumber: r.doc_number || '—',
            date: r.doc_date ? new Date(r.doc_date).toLocaleDateString('uz-Latn') : '—',
            title: r.doc_title || 'Nomsiz hujjat',
            org: r.doc_org || '—',
            excerptLabel: `Topilgan qism · ${bestChunk?.page ? bestChunk.page + '-bet' : 'matn'}`,
            excerpt: highlightExcerpt(excerptText, query),
            chunks: r.chunks,
  ...
  const RECENT_QUERIES = recentQueries.length > 0 ? recentQueries : [
          ...
          {sidebarCollapsed ? (
            ...
            <>
              ...
              <div style={{ height: 4, background: 'var(--canvas)', borderRadius: 'var(--r-pill)', marginBottom: 6, overflow: 'hidden', border: '1px solid var(--hairline)' }}>
                <div style={{ width: docCount > 0 ? '100%' : '0%', height: '100%', background: 'var(--brass-soft)', borderRadius: 'var(--r-pill)' }} />
              </div>
              <div style={{ fontFamily: 'var(--sans)', fontSize: 10, color: 'var(--ink-3)', marginBottom: 8 }}>
                {docCount > 0 ? 'Indekslangan' : 'Hujjat yuklanmagan'}
              </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {searching && (
                        <div style={{ textAlign: 'center', padding: '40px 24px' }}>
                          <div style={{ width: 24, height: 24, border: '2px solid var(--hairline)', borderTopColor: 'var(--navy)', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
                          <p style={{ fontSize: 13, color: 'var(--ink-2)' }}>Qidirilmoqda...</p>
                        </div>
                      )}
                      ...
                      {hasSearched && !noResults && !searching && (
                        <div>
                          ...
                          <p style={{ fontSize: 12, color: 'var(--ink-2)', margin: '0 0 12px', lineHeight: 1.5 }}>
                            ...
                            <span style={{ fontFamily: 'var(--mono)' }}>{searchDuration} ms</span>
                            {' '}<span style={{ color: 'var(--ink-3)' }}>·</span>{' '}
                            semantik qidiruv (vektor)
                          </p>
                    ...
                    {showPreview && (
                      <div style={{ width: 480, flexShrink: 0, position: 'sticky', top: 0, maxHeight: 'calc(100vh - 52px)', overflowY: 'auto' }}>
                        {selectedCard === null || !hasSearched ? (
                          ...
                          const chunkPreview = r.chunks?.[0]?.content?.slice(0, 400) || r.excerpt.map(p => p.text).join('')
                          return (
                            <div style={{
                              background: 'var(--surface)', border: '1px solid var(--hairline)',
                              ...
                              <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--hairline)' }}>
                                {[
                                  { label: "Moslik", value: `${r.relevance}%` },
                                  { label: "Tashkilot", value: r.org },
                                  { label: "Topilgan qismlar", value: `${r.chunks?.length || 1} ta` },
                                ].map(attr => (
